      ...
      {isSignedIn ? (
        <form onSubmit={handleSubmit} className="flex gap-2">
          <input 
            ...
            className="input input-border input-sm flex-1 bg-base-200"
            value={content}
            ...
            <div key={comment.id} className="chat chat-start">
              ...
              {currentUserId === comment.userId && (
                <div className="chat-footer">
                  <button onClick={() => confirm("Delete?") && deleteComment.mutate({commentId: comment.id})} className="btn btn-ghost btn-xs text-error" disabled={deleteComment.isPending}>
